...
gen_ins_seq <- function(code, type, node, child) {

    stopifnot(child$type == "seq")

    cpp_init <- glue("DECLARE_BINDING(rastr_{NODE}_ins, 5),",
                     .sep = "\n",
                     NODE = node,
                     CHILD = child_name)
    ...
    cpp_ifc <- glue("// ins",
                    "void rastr_{NODE}_ins(rastr_ast_t ast, rastr_node_t node, int index, int len, rastr_node_t* seq);",
                    "SEXP r_rastr_{NODE}_ins(SEXP r_ast, SEXP r_node, SEXP r_index, SEXP r_len, SEXP r_seq);",
                    .sep = "\n",
                    NODE = node,
                    CHILD = child_name)
    ...
    cpp_impl <- glue("// ins",
                     ...
                     "    int id = ptr->id;",
                     "",
                     "    int orig_len = ptr->node.{NODE}_node.len;",
                     "    rastr_node_t* orig_seq = ptr->node.{NODE}_node.seq;",
                     "",
                     "    int tot_len = orig_len + len;",
                     "    rastr_node_t* tot_seq = (rastr_node_t*) std::realloc(orig_seq, tot_len);",
                     "",
                     "    ptr->node.{NODE}_node.len = tot_len;",
                     "    ptr->node.{NODE}_node.seq = tot_seq;",
                     "",
                     ...
                     "    for(int i = 0; i < len; ++i) {{",
                     "        ASSIGN_SEQ_CHILD({NODE}, i + index, seq[i], id)",
                     "    }}",
                     "",
                     "}}",
                     "",
                     ...
                     "    }}",
                     "",
                     "    rastr_{NODE}_ins(ast, node, index, len, seq);",
                     "",
                     "    free(seq);",
                     "",
                     "    return R_NilValue;",
    ...
    r_impl <- glue("#' @export",
                   "{NODE}_ins <- function(ast, node, index, len, seq) {{",
                   "    invisible(.Call(C_rastr_{NODE}_ins, ast, node, index, len, seq))",
                   "}}",
                   .sep = "\n",
                   NODE = node,
                   CHILD = child_name)
...
gen_child <- function(code, type, node, child) {
    ...
    if(child$type == "seq") {
        ...
        gen_ins_seq(code, type, node, child)
    }
    ##child_type <- as_c_type(child$type)
    ##
    ##retcode <- switch(child$type,
